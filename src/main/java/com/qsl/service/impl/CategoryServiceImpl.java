...
import com.qsl.mapper.CategoryMapper;
import com.qsl.mapper.NewsCategoryRefMapper;
...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void deleteCategory(Integer id) {
        try {
            ...
            newsCategoryRefMapper.deleteByCategoryId(id);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<Category> listCategory() {
        ...
        try {
            ...
            e.printStackTrace();
            log.error("根据新闻获得分类列表失败, cause:{}", e);
        }
        return categoryList;
    }
    ...
    public List<Category> listCategoryWithCount() {
        ...
        try {
            ...
            for (int i = 0; i < categoryList.size(); i++) {
                Integer count = newsCategoryRefMapper.countNewsByCategoryId(categoryList.get(i).getCategoryId());
                categoryList.get(i).setNewsCount(count);
            }
        } catch (Exception e) {
            e.printStackTrace();
            log.error("根据新闻获得分类列表失败, cause:{}", e);
